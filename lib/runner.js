...
module.exports = function(options){
  ...
  this.addResult = function(step){
    ...
    if(step.options.maxSingleMean && result.stats.singleMean > step.options.maxSingleMean){
      var err = {
        code: 'maxSingleMeanExceeded',
        details: step.name,
        message: 'Mean across all concurrent requests should be below ' + step.options.maxSingleMean
      };

      this.emit('error', err);

      if(!result.errors['maxSingleMeanExceeded'])
        result.errors['maxSingleMeanExceeded']=[];

      result.errors['maxSingleMeanExceeded'].push(err);
    }
